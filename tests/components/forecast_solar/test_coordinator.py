...
async def test_coordinator_resolves_azimuth_sensor_on_setup(
    hass: HomeAssistant,
    sensor_plane_config_entry: MockConfigEntry,
) -> None:
    """Test the coordinator reads the azimuth sensor's value at setup."""
    hass.states.async_set("sensor.roof_azimuth", "142", {"unit_of_measurement": "°"})
    ...
    assert coordinator.forecast.azimuth == 142 - 180
...
async def test_coordinator_rereads_azimuth_sensor_on_each_update(
    ...
    """Test a changed sensor value is picked up on the next refresh."""
...
@pytest.mark.usefixtures("mock_forecast_solar")
async def test_coordinator_tracks_home_location_on_setup(
    hass: HomeAssistant,
    tracked_location_config_entry: MockConfigEntry,
) -> None:
    """Test the coordinator falls back to HA's home location when none is stored."""
    ...
    assert coordinator.forecast.longitude == -0.1
...
async def test_coordinator_retracks_home_location_on_update(
    ...
    """Test a changed home location is picked up on the next refresh."""
    hass.config.latitude = 51.5
    hass.config.longitude = -0.1
    tracked_location_config_entry.add_to_hass(hass)
    await hass.config_entries.async_setup(tracked_location_config_entry.entry_id)
